//
//  TrackingResultSelectCarrierCell.m
//  YQTrack
//
...
@interface TrackingResultSelectCarrierCell()<UIGestureRecognizerDelegate>
/**
 *  用图片的时候显示运输商图片
 */
...
@implementation TrackingResultSelectCarrierCell
...
-(void)resetValueWithRecordModel:(TrackRecordModel*)recordModel isDestination:(BOOL)isDestination{
    ...
    self.statusLabel.font = [UIFont fontWithName:iconFontName size:12];
...
//        carrierLoder = [[YQResourceManager sharedSingleton] loaderForResourceType:ResourceTypeGCarrier];
    ...
    if(carrierLogo == nil){
        ...
        self.carrierIconImageView.tintColor = [UIColor colorWithHexString:@"969696" alpha:1];
        [self.carrierIconImageView setImage:[[UIImage imageNamed:@"trackResult_selectCarrier"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate]];
    }
    //显示图片
    else if([carrierLogo isKindOfClass:[UIImage class]]){
        self.carrierIconImageView.hidden    = NO;
        self.carrierLogoLable.hidden        = YES;
        self.carrierIconImageView.tintColor = nil;
...
-(void)setIsMutableCarrier:(BOOL)isMutableCarrier{
    ...
    if(_isMutableCarrier){
        ...
        [self.carrierCountryLabel updateConstraints:^(MASConstraintMaker *make){
            make.bottom.equalTo(lineSuperView.bottom).offset(-12);
        }];
...
//        [self.informationImage setImage:[UIImage imageNamed:@"trackResult_information_ valid"]];
        self.informationLabel.textColor = [UIColor colorWithHexString:@"4596f7" alpha:1];
        
    }
    else{
        self.informationLabel.textColor = [UIColor colorWithHexString:@"000000" alpha:0.54];
    }
}
...
-(void)resetValueWithCarrierNumber:(NSNumber*)number{
    ...
    NSString *carrierString = @"";
    if(carrierInteger == 0){
        //        carrierString = [number stringValue];
